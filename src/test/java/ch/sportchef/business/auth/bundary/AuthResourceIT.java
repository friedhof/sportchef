 ...
 */
package ch.sportchef.business.auth.bundary;
...
public class AuthResourceIT {
    ...
    @Rule
    public final JAXRSClientProvider provider = buildWithURI("http://localhost:8080/sportchef/api/authenticate");
...
*/
    private void loginWithSuccess() {
        // arrange
        /**
         * @ToDo: get E-Mail from test user without creating a User
         */
        final String email = "patrick@reimers.me";
        final JsonObject loginData = Json.createObjectBuilder()
                .add("email", email)
                .build();

        // act
        final Response response = this.provider.addPath("/login/"+email).request(MediaType.APPLICATION_JSON).post(Entity.json(loginData));
...
//        final String cookie = cookieData.toString();
    ...
    private void loginWithWrongEmail() {
        // arrange
        final String email = "mail-does-not-exist@sportchef.ch";
        final JsonObject loginData = Json.createObjectBuilder()
                .add("email", email)
                .build();
